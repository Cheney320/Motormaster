...
def batch_beam_decode(model, batch, vocab, params):
    ...
    def decode_onestep(enc_inp, enc_outputs, dec_input, dec_state):
        ...
        top_k_probs, top_k_ids = tf.nn.top_k(tf.squeeze(final_dists), k=params["beam_size"])
    ...
    while steps < params['max_dec_steps'] and len(results) < params['beam_size']:
        ...
        topk_ids, topk_log_probs, new_states, attn_dists = returns['top_k_ids'],\
    ...
    # best_hyp.abstract = " ".join(output_to_words(best_hyp.tokens, vocab, batch[0]["article_oovs"])[1:-1])
